...
## [2.7.0-gd.2] - 2026-09-01

### 优化
- 为 `skills_like` 模式分层长期记忆工具说明：首轮仅保留调用场景，关键词组织、重试、记忆格式和重要度规则延迟到参数 schema。

## [2.7.0-gd.1] - 2026-08-29

### 新增
- **记忆库定期整合**（移植上游 2.6.0-beta.2，默认关闭）：把零散的低价值记忆聚合、总结为更精炼的一条，从源头控制记忆库规模。支持同会话聚合与跨会话语义聚类（复用已存向量批量搜索，不产生额外 Embedding 调用）；旧记忆可归档（保留原文、移出检索索引）或删除；触发方式支持每日定时或反思时执行（6 小时冷却）；新增 `/lmem consolidate` 手动触发。
- **记忆总结语言** 新增 `memory_language` 配置（默认中文，行为不变）：`mixed` 中英混合模式下总结主体用中文，但对话中的英文原词/原句照录不译并写入 key_facts，适合练习英语时记住自己用过的表达。注意：`bot_language` 仅控制插件界面/回复语言，与记忆生成语言无关。
- **嵌入模型指纹**：记录当前 Embedding Provider（ID+模型+维度），模型变化（即使维度相同）时自动触发全量向量重建，避免旧模型向量静默残留。

### 修复
- **会话重置信号兼容**（移植上游 #244）：AstrBot 已将 `/reset`、`/new` 的清理信号改名为 `_clean_group_context_session`，本插件此前只监听旧名，导致重置后插件会话上下文未清空。现在两个信号均识别。
- **记忆注入事实去重**（移植上游 #202/#223）：注入展示优先使用人格化摘要通道；旧版 v2 记录按精确后缀剥离正文中的关键事实，消除与 `Key facts` 元数据行的重复注入。检索语料与 Agent 工具返回内容不变。
- **图边写入竞态**：`_add_edge` 改为 `INSERT ... ON CONFLICT(edge_key)` 原子写入；语义合并的权重累积按边权重缩放（此前固定 +0.15）。
- **图 FTS 检索兜底**：非法 FTS 查询不再中断整条图路由检索。
- **删除崩溃恢复补全**：删除操作的写日志重放现在会补删 documents 表与向量/BM25 索引，避免崩溃后记忆仍可检索但图/原子已删除。
- **访问时间原子化**：召回路径的访问时间/访问计数更新合并为单条原子 SQL（`json_set`），消除并发读改写丢失更新，并按整批一次提交。
- **会话/原子并发安全**：`create_session` 幂等化（`ON CONFLICT DO NOTHING`）；`trim_session_messages` 的读改写整体纳入写锁；原子 `reinforce` 使用 `BEGIN IMMEDIATE` 抢占写锁。
- **迁移与启动修复**：版本表存在但为空的全新数据库不再误判为 v1 跑完整迁移链；v7→v8 仅回填缺失的 `access_count` 字段；写操作日志重试耗尽后正确进入终态 `failed`。
- **依赖**：`requirements.txt` 移除 `faiss-cpu` 固定（由 AstrBot 核心提供），避免与 AstrBot 锁定版本冲突。

### 性能
- **图向量按记忆压缩**（移植上游 2.4.1）：图向量索引从每个图条目一个向量收敛为每条来源记忆一个聚合向量，索引体积与写放大幅下降（本库实测主索引 3.7MB / 图索引 65MB，压缩后预期显著缩小）；`/lmem rebuild-graph` 可重建为新结构。人物节点同时改用「平台:发送者ID」稳定标识，昵称变化不再分裂节点（移植上游 2.5.1 #224）。
- **每日衰减**逐行 JSON 计算卸载到线程池，不再阻塞事件循环。
- **统计与清理**改为主键 keyset 流式分页，替代 O(N²) OFFSET 分页。
- **会话记忆查询**改为单条 SQL 过滤+排序+分页。
- **一致性检查**的 BM25 缺失计数改为 SQL 层计算，避免加载全量 ID 集合。
- **影子索引安全重建**（移植上游 2.5.2）：`/lmem rebuild-index` 的 BM25 全量重建写入影子表并原子切换，重建期间检索不中断；向量重建持久化影子索引与断点进度，中断后可续跑；重建不再长时间持有全局数据库锁；完成后对并发写入的新文档做有上限的补偿修复。
- **启动维护后台化**：索引一致性检查与自动重建调度到后台执行，不再阻塞插件启动；`/lmem status` 显示维护状态。

### 说明
- 版本表结构无变化（仍为 v8），旧数据直接升级即可；图向量压缩与人物身份稳定仅影响升级后新写入/重建的数据，`/lmem rebuild-graph` 可全量重建。
- 记忆整合默认关闭，需在配置 `memory_consolidation` 中开启。

## [2.6.0-gd.3] - 2026-08-19

### 修复
- 情绪事实不再在 240 字处直接切断；存储保留最多 600 字，超限时优先在完整标点处收束。
- 私聊总结提示词要求情绪事实保持精炼并以完整句子结束，避免下游注入出现残句。
