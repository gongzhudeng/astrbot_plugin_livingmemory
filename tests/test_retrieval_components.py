...
async def test_hybrid_retriever_fallback_when_one_channel_fails():
    ...
    relevance_by_id = {result.doc_id: result.relevance_score for result in results}
    assert relevance_by_id[1] == pytest.approx(0.7)
    assert relevance_by_id[2] == pytest.approx(0.95)
