...
@pytest.mark.asyncio
async def test_handle_memory_recall_filters_below_relevance_threshold(
    ...
    memory_engine.search_memories = AsyncMock(return_value=[accepted, rejected])
    ...
    assert "meeting memory" in injected
    assert "coffee preference" not in injected


@pytest.mark.asyncio
async def test_handle_memory_recall_skips_when_all_below_relevance_threshold(
    handler, memory_engine
):
    event = _make_event(group=False)
    req = _make_req("query text")
...
def test_legacy_spark_retrieval_query_detection():
    from astrbot_plugin_livingmemory.core.event_handler_modules.message_utils import (
        MessageUtils,
    )
    ...
    proactive_query = (
        ...
        "[主动轮] 用户未发送新消息\n"
        "AI：最近还好吗？\n\n"
        "[检索提示：用户最近的经历、近况、未完成的话题]"
    )
    normal_query = "我看到你说了最近聊天：你最近还好吗？"

    assert MessageUtils.is_legacy_spark_retrieval_query(proactive_query) is True
    assert (
        MessageUtils.is_legacy_spark_retrieval_query(
            "[检索提示：用户最近的经历、近况、未完成的话题]"
        )
        is True
    )
    assert MessageUtils.is_legacy_spark_retrieval_query(normal_query) is False


@pytest.mark.asyncio
async def test_storage_task_excludes_legacy_spark_retrieval_query(
    ...
    summarized_messages = memory_processor.process_conversation.await_args.kwargs[
        "messages"
    ]
    assert [message.content for message in summarized_messages] == ["真实的主动回复"]
