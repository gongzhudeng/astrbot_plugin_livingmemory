...
class DualRouteRetriever:
    ...
    async def search(
        ...
        for doc_id in all_doc_ids:
            ...
            document_relevance = (
                float(doc_result.relevance_score) if doc_result is not None else 0.0
            )
            graph_relevance = (
                float(graph_result.relevance_score) if graph_result is not None else 0.0
            )
            relevance_score = max(
                0.0,
                min(1.0, max(document_relevance, graph_relevance)),
            )
            final_score = min(
                1.0,
