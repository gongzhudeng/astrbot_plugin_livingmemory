...
class RecallEngineConfig(BaseModel):
    ...
    top_k: int = Field(
        ...
        ge=0,
        le=50,
        description="最多返回的记忆数量。设为 0 则跳过自动召回和注入",
    )
    min_relevance_score: float = Field(
        default=0.5,
        ge=0.0,
        le=1.0,
        description="自动注入记忆的最低独立相关度。设为 0 则关闭过滤",
    )
    max_k: int = Field(
...
class ProviderConfig(BaseModel):
    ...
    llm_fallback_provider_ids: list[str] = Field(
        default_factory=list,
        description="按顺序使用的 LLM 兜底 Provider ID 列表",
    )
