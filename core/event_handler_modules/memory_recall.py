...
class MemoryRecall:
    ...
    @staticmethod
    def _get_relevance_score(memory) -> float:
        raw_score = getattr(memory, "relevance_score", None)
        if not isinstance(raw_score, (int, float)):
            raw_score = getattr(memory, "final_score", 0.0)
        try:
    ...
    async def handle_memory_recall(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        try:
            ...
            async with OperationContext("记忆召回", session_id):
                ...
                event_extra = get_extra if callable(get_extra) else lambda _key: None
                is_proactive_retrieval = (
                    event_extra("spark_proactive_retrieval") is True
                )
                is_background_retrieval = event_extra("background_retrieval") is True
                background_query = event_extra("retrieval_query")
                if is_background_retrieval and isinstance(background_query, str):
                    actual_query = background_query.strip()
                    request_query = actual_query

                # Store only genuine user messages, never synthetic retrieval queries.
                is_group = event.get_message_type() == MessageType.GROUP_MESSAGE
                if (
                    not is_group
                    and actual_query
                    and not is_proactive_retrieval
                    and not is_background_retrieval
                ):
                ...
                query_for_search = (
                    request_query
                    if is_proactive_retrieval or is_background_retrieval
                    else actual_query
                )

                # Background retrieval already carries its exact context window.
                if (
                    not is_proactive_retrieval
                    and not is_background_retrieval
                    ...
                    try:
                        ...
                        if recent_messages and len(recent_messages) > 1:
                            ...
                            for msg in reversed(recent_messages[1:]):
                                ...
                                if self.message_utils.is_legacy_spark_retrieval_query(
                                    normalized_content
                                ):
                                    continue
                                context_parts.append(normalized_content)
                            if context_parts:
                ...
                if recalled_memories:
                    candidate_count = len(recalled_memories)
                    min_relevance = self.config_manager.get(
                        "recall_engine.min_relevance_score", 0.5
                    )
                    recalled_memories = [
                        memory
                        for memory in recalled_memories
                        if self._get_relevance_score(memory) >= min_relevance
                    ]
                    logger.info(
                        f"[{session_id}] 记忆相关度准入: 候选={candidate_count}, "
                        f"通过={len(recalled_memories)}, 阈值={min_relevance:.2f}"
                    )

                if recalled_memories:
                    logger.info(
                        f"[{session_id}] 将注入 {len(recalled_memories)} 条相关记忆"
                    )
                    ...
                    for i, mem in enumerate(recalled_memories, 1):
                        relevance_score = self._get_relevance_score(mem)
                        logger.debug(
                            f"[{session_id}] 记忆 #{i}: 相关度={relevance_score:.3f}, "
                            f"综合分={mem.final_score:.3f}, "
                    ...
                    if is_background_retrieval:
                        results = event_extra("background_retrieval_results")
                        if not isinstance(results, list):
                            results = []
                        results.append(memory_str)
