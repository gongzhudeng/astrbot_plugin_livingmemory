...
class MessageUtils:
    ...
    @staticmethod
    def is_legacy_spark_retrieval_query(content: str) -> bool:
        """Identify Spark retrieval-control text accidentally stored as user chat."""
        normalized = str(content or "").strip()
        if not normalized:
            return False

        standalone_prefixes = ("[检索提示：", "[模板指令：")
        if normalized.startswith(standalone_prefixes) and normalized.endswith("]"):
            return True

        if not normalized.startswith("最近聊天："):
            return False

        has_rendered_turn = "\nAI：" in normalized and (
            "\n用户：" in normalized or "[主动轮] 用户未发送新消息" in normalized
        )
        has_retrieval_control = any(
        ...
        return has_rendered_turn and has_retrieval_control
